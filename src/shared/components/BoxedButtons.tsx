...
const BoxedButtons: React.FC<Props> = ({ buttons, classNameProp }) => (
  <View
    className={`flex-row items-center justify-center rounded-lg border border-[#414141] overflow-hidden ${classNameProp}`}
  >
    {buttons.map(({ key, icon, label, onPress }, index) => (
      <TouchableOpacity
        key={key}
        onPress={onPress}
        className={`${index > 0 && 'border-l border-[#414141]'}`}
      >
        <View key={key} className="flex-row items-center justify-between p-3">
          {icon}
          {label && <DText className={`${icon && 'pl-2'}`}>{label}</DText>}
        </View>
      </TouchableOpacity>
    ))}
  </View>
